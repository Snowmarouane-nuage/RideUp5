    ...
    <div className="min-h-screen bg-black text-white pt-28 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
          <div>
            <div className="text-[#9AB8FF] font-display text-xs tracking-[0.3em] mb-2">DASHBOARD</div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-8">
              ...
              <div className="p-6 border-2 border-[#9AB8FF] bg-gradient-to-br from-[#9AB8FF]/15 to-transparent">
                <div className="font-display text-xs tracking-widest text-[#9AB8FF] mb-3 flex items-center gap-2">
                ...
                <Link to="/coach" className="mt-4 inline-flex items-center gap-2 text-[#9AB8FF] text-xs font-display tracking-wider hover:gap-3 transition-all">
            ...
            {!stats.has_coach_profile && (
              <div className="p-6 border-2 border-dashed border-[#9AB8FF]/50 bg-[#9AB8FF]/5 mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                ...
                <Link data-testid="cta-coach-onboarding" to="/coach" className="bg-[#9AB8FF] hover:bg-[#7A9CE8] text-white px-5 py-3 font-display tracking-wider whitespace-nowrap">
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {stats.recent_analyses.map((a, i) => (
                    <Link key={i} to="/video-analysis" className="p-4 border border-[#262626] bg-[#0A0A0A] hover:border-[#9AB8FF]/50 transition">
                      <div className="flex items-center justify-between text-xs text-gray-400 mb-2">
                        <span className="font-display text-[#9AB8FF] tracking-wider">{a.sport.toUpperCase()} · {a.level.toUpperCase()}</span>
...
function StatCard({ icon, label, value }) {
  return (
    <div className="p-4 border border-[#262626] bg-[#0A0A0A]">
      <div className="flex items-center gap-2 text-[#9AB8FF] mb-2">{icon}<span className="font-display text-[10px] tracking-widest text-gray-400">{label}</span></div>
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} className="flex-1 flex flex-col items-center justify-end gap-2">
            ...
            <div
              className={`w-full transition-all duration-500 ${isCurrent ? "bg-[#9AB8FF]" : "bg-[#9AB8FF]/40"}`}
  ...
  return (
    <Link
      ...
      className={`group relative block p-6 border transition-all hover:-translate-y-1 ${
        locked ? "border-[#262626] bg-[#0A0A0A] opacity-80" : "border-[#262626] bg-[#0A0A0A] hover:border-[#9AB8FF]/60"
      }`}
    >
      {premium && (
        <div className="absolute top-3 right-3 text-[10px] font-display tracking-widest px-2 py-1 bg-[#9AB8FF] text-white">PREMIUM</div>
      )}
      <div className="text-[#9AB8FF] mb-3">{locked ? <Lock className="h-6 w-6" /> : icon}</div>
      <div className="font-display text-xl mb-1">{title}</div>
      <p className="text-gray-400 text-sm mb-4">{desc}</p>
      <div className="inline-flex items-center gap-2 text-xs font-display tracking-wider text-[#9AB8FF] group-hover:gap-3 transition-all">

...
import { Sparkles, Lock, Check, Send, Target, Award, MessageCircle, ArrowRight } from "lucide-react";
    ...
    return (
      <div className="min-h-screen bg-black text-white pt-28 pb-20 px-6">
        <div className="max-w-2xl mx-auto text-center p-12 border border-[#262626] bg-[#0A0A0A]">
          ...
          <h1 className="font-display text-3xl md:text-5xl mb-4">COACH PERSONNEL</h1>
          <p className="text-gray-400 mb-8">Ton coach IA dédié pour progresser trick par trick. Réservé aux abonnés.</p>
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black text-white pt-28 px-6 flex items-center justify-center">
        <div className="h-12 w-12 rounded-full border-4 border-[#1E6BFF] border-t-transparent animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white pt-28 pb-20 px-6">
      <div className="max-w-5xl mx-auto">
        <div className="text-[#1E6BFF] font-display text-xs tracking-[0.3em] mb-2">COACH IA</div>
        <h1 className="font-display text-4xl md:text-6xl mb-3">TA <span className="text-[#1E6BFF]">PROGRESSION</span></h1>
        <p className="text-gray-300 max-w-3xl mb-8 italic">{profile.welcome}</p>
...
function TabBtn({ active, onClick, children, testId }) {
  return (
    <button
      data-testid={testId}
      onClick={onClick}
      className={`px-4 py-3 font-display tracking-wider text-sm flex items-center gap-2 transition border-b-2 -mb-px ${active ? "border-[#1E6BFF] text-white" : "border-transparent text-gray-400 hover:text-white"}`}
    >
      {children}
    </button>
  );
  ...
  return (
    <div className="min-h-screen bg-black text-white pt-28 pb-20 px-6">
      <div className="max-w-2xl mx-auto">
        <div className="text-[#1E6BFF] font-display text-xs tracking-[0.3em] mb-2">COACH IA · ONBOARDING</div>
        <h1 className="font-display text-4xl md:text-5xl mb-6">FAISONS <span className="text-[#1E6BFF]">CONNAISSANCE</span></h1>
            ...
            <div className="grid grid-cols-2 gap-3">
              {["Débutant", "Intermédiaire", "Avancé", "Pro"].map((lvl) => (
                <button
                  ...
                  className={`p-4 border font-display tracking-wider transition ${data.level === lvl ? "border-[#1E6BFF] bg-[#1E6BFF]/15" : "border-[#262626] hover:border-[#1E6BFF]/50"}`}
                >
                  {lvl.toUpperCase()}
                </button>
              ))}
            </div>
            <button data-testid="ob-next-0" onClick={() => setStep(1)} className="w-full bg-[#1E6BFF] hover:bg-[#1751C4] py-3 font-display tracking-wider mt-4">SUITE →</button>
          </div>
        )}
          ...
          <div className="p-8 border border-[#262626] bg-[#0A0A0A] space-y-5" data-testid="onboarding-step-1">
            ...
            <div className="flex flex-wrap gap-2 min-h-[40px]">
              {data.current_tricks.map((t, i) => (
                <span key={i} className="inline-flex items-center gap-1 px-3 py-1.5 bg-[#1E6BFF]/15 border border-[#1E6BFF]/50 text-sm">
                  {t}
                  <button type="button" onClick={() => setData({ ...data, current_tricks: data.current_tricks.filter((_, idx) => idx !== i) })} className="text-[#1E6BFF] hover:text-white text-base leading-none ml-1">×</button>
                </span>
              ))}
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-black border border-[#262626] px-3 py-2 outline-none focus:border-[#1E6BFF]"
              />
              <button type="button" data-testid="add-trick" onClick={() => { if (newTrick.trim()) { setData({ ...data, current_tricks: [...data.current_tricks, newTrick.trim()] }); setNewTrick(""); } }} className="px-4 border border-[#262626] hover:border-[#1E6BFF] text-sm font-display tracking-wider">AJOUTER</button>
            </div>
            <div className="flex gap-3 mt-4">
              <button type="button" onClick={() => setStep(0)} className="px-5 py-3 border border-[#262626] hover:border-[#1E6BFF] font-display tracking-wider">← RETOUR</button>
              <button type="button" data-testid="ob-next-1" onClick={() => setStep(2)} className="flex-1 bg-[#1E6BFF] hover:bg-[#1751C4] py-3 font-display tracking-wider">SUITE →</button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="p-8 border border-[#262626] bg-[#0A0A0A] space-y-5" data-testid="onboarding-step-2">
            ...
            <textarea
              ...
              className="w-full bg-black border border-[#262626] px-3 py-3 outline-none focus:border-[#1E6BFF] resize-none"
            />
            {error && <div className="text-red-500 text-sm">{error}</div>}
            <div className="flex gap-3 mt-4">
              <button type="button" onClick={() => setStep(1)} className="px-5 py-3 border border-[#262626] hover:border-[#1E6BFF] font-display tracking-wider">← RETOUR</button>
              <button
                type="button"
                data-testid="ob-finish"
                onClick={submit}
                disabled={submitting}
                className="flex-1 bg-[#1E6BFF] hover:bg-[#1751C4] py-3 font-display tracking-wider disabled:opacity-50 flex items-center justify-center gap-2"
              >
    ...
    <div className="space-y-6" data-testid="roadmap-panel">
      ...
      <div className="p-6 border-2 border-[#1E6BFF] bg-gradient-to-br from-[#1E6BFF]/15 to-transparent">
        <div className="flex items-center justify-between mb-3">
          <div className="font-display text-xs tracking-widest text-[#1E6BFF] flex items-center gap-2"><Award className="h-3 w-3" /> PROGRESSION GLOBALE</div>
        ...
        <div className="h-2 bg-[#262626] overflow-hidden">
          <div className="h-full bg-[#1E6BFF] transition-all duration-700" style={{ width: `${pct}%` }} />
        </div>
        ...
        {roadmap.map((t, i) => {
          ...
          return (
            <div
              key={t.trick}
              data-testid={`trick-${i}`}
              className={`p-5 border ${isDone ? "border-[#262626] opacity-60" : isCurrent ? "border-[#1E6BFF] bg-[#1E6BFF]/5" : "border-[#262626]"} bg-[#0A0A0A] flex items-center gap-4`}
            >
              <div className={`w-10 h-10 flex items-center justify-center font-display ${isDone ? "bg-[#1E6BFF] text-white" : "border border-[#262626] text-gray-500"}`}>
              ...
              <div className="flex-1">
                <div className="font-display text-lg flex items-center gap-2 flex-wrap">
                  {t.trick}
                  {isCurrent && <span className="text-[10px] font-display tracking-widest bg-[#1E6BFF] text-white px-2 py-0.5">EN COURS</span>}
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {!isDone && (
                  <button
                    ...
                    className="text-xs px-3 py-2 border border-[#1E6BFF] text-[#1E6BFF] hover:bg-[#1E6BFF] hover:text-white font-display tracking-wider transition"
                  >
                    VALIDÉ
                  </button>
                )}
      ...
      {done === roadmap.length && roadmap.length > 0 && (
        <div className="p-6 border-2 border-[#1E6BFF] bg-[#1E6BFF]/10 text-center">
    ...
    <div className="space-y-4" data-testid="chat-panel">
      <div ref={scrollRef} className="h-[500px] overflow-y-auto p-6 border border-[#262626] bg-[#0A0A0A] space-y-4">
        {messages.length === 0 && (
          <div className="text-center text-gray-500 py-12">
            <MessageCircle className="h-10 w-10 text-[#1E6BFF] mx-auto mb-3" />
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[80%] p-3 ${m.role === "user" ? "bg-[#1E6BFF] text-white" : "bg-[#1a1a1a] text-gray-100 border border-[#262626]"}`}>
          ...
          <div className="flex justify-start">
            <div className="bg-[#1a1a1a] border border-[#262626] p-3">
              ...
              <div className="flex gap-1">
                <span className="h-2 w-2 bg-[#1E6BFF] rounded-full animate-bounce" style={{ animationDelay: "0ms" }}></span>
                <span className="h-2 w-2 bg-[#1E6BFF] rounded-full animate-bounce" style={{ animationDelay: "120ms" }}></span>
                <span className="h-2 w-2 bg-[#1E6BFF] rounded-full animate-bounce" style={{ animationDelay: "240ms" }}></span>
              </div>
            </div>
          </div>
        )}
      ...
      <form onSubmit={send} className="flex gap-2">
        <input
          ...
          className="flex-1 bg-black border border-[#262626] px-4 py-3 outline-none focus:border-[#1E6BFF]"
        />
        <button data-testid="chat-send" type="submit" disabled={sending || !input.trim()} className="px-5 bg-[#1E6BFF] hover:bg-[#1751C4] disabled:opacity-50 transition flex items-center gap-2 font-display tracking-wider">
